...
import {Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Dialog, DialogTitle, DialogContent, DialogContentText, TextField, Slide, CircularProgress, Tooltip } from '@material-ui/core';
...
const GenerateYourLink: React.FC<RouteComponentProps> = (props: RouteComponentProps) => {
  ...
  const preventNumericSpecialCharacter = (e: any) => {
    if (e.key === 'e' || e.key === '-') {
      e.preventDefault();
    }       
  }
                  ...
                  <TableCell className={`${styles.TableCellBody} ${styles.TableCellBodyFlex}`} align="right">
                    {
                      row.linkGenerated &&  (
                        <Tooltip title={<p className={styles.valueGroupShowTooltip}>Copy to clipboard</p> }>
                          <CopyToClipboard onCopy={() => NotificationManager.success("Copied")} text={`${BASE_URL}/#/buy-token?campaignId=${transactionHash}${affiliate ? `&referral=${loginUser}&campaignIndex=${row.campaign_index}`: ''}`}>
        ...
        }
        {
          ...
          >
              ...
              <TextField
                ...
                onKeyDown={preventNumericSpecialCharacter}
                fullWidth
              />
              {
